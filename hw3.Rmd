...
ui <- fluidPage(
  ...
  theme = shinythemes::shinytheme( 'superhero' ),
  sidebarPanel(
    selectInput('icd', 'International Classification of Disease', unique(res$ICD.Chapter), selected='Certain infectious and parasitic diseases'),
    selectInput('state', 'State', unique(res$State), selected='AL'),
    selectInput('year', 'Year', unique(res$Year), selected='1999')
  ),
  mainPanel( 
    h4('Compare State-Level Crude Mortality Rates to the National Rates'),
    h6('Bar Chart per State'),
    p("From the drop-down menu to the left, chose a International Classification of Disease and State of interest to observe how a given state's mortality rate compares to the national average across all years for which data is present" ),
    h6('Map View per Year'),
    p("From the drop-down menu to the left, chose a International Classification of Disease and Year of interest to observe the state-level deviation of mortality rate from the national average for a given year" ),
    tabsetPanel(
      tabPanel( 'Bar Chart per State', plotOutput('plot1')),
      tabPanel( 'Map View per Year', plotOutput('plot2'))
      #tabPanel( 'Treemap View', plotOutput('plot3'))
  ))
)
